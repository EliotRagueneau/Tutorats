...
def affichermenu():
    print(' Indiquez votre deplacement avec "zqsd"')
    print(' z: pour aller vers le haut')
    print(' q: pour aller vers la gauche')
    print(' s: pour aller vers le bas')
    print(' d: pour aller vers la droite')
    print(' --------------------------------')
...
def deplahaut(grille, yjoueur, xjoueur, contenupre):
    if (grille[yjoueur - 1][xjoueur] != " "):
        ...
        contenupre = grille[yjoueur - 1][
            xjoueur]  # "contenupre" va prendre le carractère ASCII de la case où le personnage va arrriver
        yjoueur = yjoueur - 1
        xjoueur = xjoueur
...
def init_salles(grille, salles):
    serierandom = list(range(0, 12))
    sallesrempli = {"maitrech": 1, "savantfou": 2, "Bibb1": 3, "Bibb2": 4, "Bibb3": 5, "pinte1": 6, "pinte2": 7,
                    "pinte3": 8, "pinte45": 9, "vide": 10, "vide": 11, "vide": 12}
    ...
    for cle2 in list(sallesrempli.keys()):
        ...
        if yjoueur == (sallesrempli[cle2]["y"]) and xjoueur == (sallesrempli[cle2]["x"]):
            ...
            if cle2 in ['Bibb1', 'Bibb2', 'Bibb3']:
                print("Vous avez rencontre un Bibbindum Chamallow vous perdez 2  pinte d'energie")
                venergie = venergie - 2
...
while (True):
    ...
    grille, yjoueur, xjoueur, contenupre, sallesremp, venergie, resu = interaction(grille, yjoueur, xjoueur, contenupre,
                                                                                   sallesremp, venergie)
    while resu != 1:
        grille, yjoueur, xjoueur, contenupre, sallesremp, venergie, resu = interaction(grille, yjoueur, xjoueur,
                                                                                       contenupre, sallesremp, venergie)
